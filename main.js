...
(async function() {
    ...
    try {
      const data = await d3.csv('https://raw.githubusercontent.com/tamimuiuc/airbnb-narrative-vis-dataset/main/data/USA-Airbnb-dataset.csv');
      ...
      const us = await d3.json('https://raw.githubusercontent.com/tamimuiuc/airbnb-narrative-vis-dataset/main/data/state.json');
      ...
      // Define a tooltip
      var tooltip = d3.select("#scene2").append('div')
        .attr('class', 'tooltip')
        .style('opacity', 0);
  
      // Draw the map
      var svg = d3.select("#scene2-graph")
        .append("svg")
        .attr("width", 960)
        .attr("height", 600);
      ...
      var axis = d3.axisBottom(legendScale)
          ...
          .tickFormat(d => Math.round(d / 1000) + 'K');  // Round the numbers and add 'K' to denote thousands
        ...
        svg.selectAll("myRect")
            ...
            .on("mouseover", function(event, d) {  // Add interactivity
                tooltip.style('visibility', 'visible')
                    .html(d.key + "<br/>" + "Average price: " + d.value.toFixed(2));
            })
...
(async function() {
  ...
  try {
    ...
    // Create tooltip
    let tooltip = d3.select('#scene4-graph').append('div')
      .attr('class', 'tooltip')
      .style('opacity', 0);
    ...
    points
      .on('mouseover', function (event, d) {
        ...
        tooltip.html(`<strong>Price:</strong> $${d.price}<br><strong>Reviews:</strong> ${d.number_of_reviews}<br><strong>Room Type:</strong> ${d.room_type}`)
          .style('left', (event.pageX) + 'px')
          .style('top', (event.pageY - 28) + 'px');
      })
      .on('mouseout', function (d) {
    ...
    // vertical line
    chart.append('path')
      .attr('d', line([[centerX, 0], [centerX, chartHeight]]))
      .attr('stroke', 'grey')
      .attr('stroke-dasharray', '4')
      .attr('fill', 'none');
    // Add 10% to centerY for horizontal line position
    const lineYPos = centerY + (0.34 * chartHeight); 

    // horizontal line
    chart.append('path')
      .attr('d', line([[0, lineYPos], [chartWidth, lineYPos]]))
      .attr('stroke', 'grey')
      .attr('stroke-dasharray', '4')
      .attr('fill', 'none');
    // Add annotations
      ...
      .attr('x', centerX / 4 * 7)
      .attr('y', centerY / 2)
      .text('High price, High reviews (Premium)')
      .attr('text-anchor', 'middle')
      .style('fill', 'grey')
    ...
    chart.append('text')
      .attr('x', centerX / 4 * 7)
      .attr('y', centerY / 4 * 6.9)
      .text('High price, Low reviews (Overpriced)')
      .attr('text-anchor', 'middle')
      .style('fill', 'grey')
      .style('font-size', '10px');
    ...
    chart.append('text')
      .attr('x', centerX / 1.45)
      .attr('y', centerY / 2)
      .text('Low price, High reviews (Value for money)')
      .attr('text-anchor', 'middle')
      .style('fill', 'grey')
      .style('font-size', '10px');
    ...
    chart.append('text')
      .attr('x', centerX / 1.45)
      .attr('y', centerY / 4 * 6.9)
      .text('Low price, Low reviews (Potential issues)')
      .attr('text-anchor', 'middle')
      .style('fill', 'grey')
      .style('font-size', '10px');
...
(async function() {
    ...
    d3.selectAll('.scene').style('display', 'none');
    // Show this scene
    d3.select('#scene7').style('display', 'block');
    ...
    try {
      ...
      // Create a tooltip
      var tooltip = d3.select("#scene7-graph").append('div')
        .attr('class', 'tooltip')
        .style('opacity', 0);
